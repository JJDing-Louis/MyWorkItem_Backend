...
本專案使用 .NET 10，方案檔為 `MyWorkItem.Backend.sln`。原始碼、測試與資料庫
Migration 分別位於 `src/`、`tests/` 與 `src/MyWorkItem.DatabaseMigrator/Scripts/`。

## 技術方向

- 使用 ASP.NET Core Web API 與 C#。
- 資料存取使用 Dapper、SqlKata 與 Microsoft.Data.SqlClient，不加入 EF Core。
- 密碼使用 `PasswordHasher<Account>`，但不使用 Identity Entity 或 EF Store。
- 資料庫 Schema 使用 DbUp SQL Migration 管理。
...
├── MyWorkItem.Infrastructure/  # Dapper、SqlKata、JWT 與外部服務實作
...
- API 不直接公開資料庫模型，應使用明確的 Request／Response DTO。
...
- SqlKata 負責組合參數化查詢，Dapper 負責執行與映射；禁止共用長生命週期連線。
...
- 使用 Dapper 參數與 SqlKata，禁止串接使用者輸入產生 SQL。
...
- 整合測試使用 SQL Server Testcontainers 驗證 Migration、查詢與 HTTP 行為。
